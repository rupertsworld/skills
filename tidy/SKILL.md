...
- After `git worktree remove` succeeds, also remove any flat-dir symlink at `~/Repos/worktrees/<name>` (only if it's a symlink, not a real directory) — it's now dangling.
